const STATIC_CACHE = "aponar-nihon-static-v7";
const DYNAMIC_CACHE = "aponar-nihon-dynamic-v7";
...
@media (max-width:380px){
  ...
  .app-tool-icon b{font-size:14px!important}.app-tool-item strong{font-size:9px!important;min-height:21px!important}.app-tool-item small{font-size:6.8px!important;min-height:14px!important}
}
</style>`;
...
<section class="an-student-power" id="student-power-tools" aria-label="Student Power Tools">
  <div class="an-sp-card">
    <div class="an-sp-head"><div><small>STUDENT POWER TOOLS</small><h2>পড়া + জাপান জীবন সহজ করুন</h2></div><a class="an-sp-all" href="student-tools.html">সব টুল →</a></div>
    <div class="an-sp-grid">
      <a class="an-sp-item" href="student-tools.html#listening"><b>🎧</b><strong>Listening</strong><small>Shadowing Lab</small></a>
      <a class="an-sp-item" href="student-tools.html#flashcards"><b>🧠</b><strong>Flashcards</strong><small>Smart SRS</small></a>
      <a class="an-sp-item" href="student-tools.html#survival"><b>🇯🇵</b><strong>Survival</strong><small>Real-life practice</small></a>
      <a class="an-sp-item" href="student-tools.html#planner"><b>📅</b><strong>JLPT Planner</strong><small>Daily roadmap</small></a>
      <a class="an-sp-item" href="student-tools.html#mistakes"><b>📝</b><strong>ভুল খাতা</strong><small>Auto review</small></a>
      <a class="an-sp-item" href="student-tools.html#toolkit"><b>🧰</b><strong>Student Toolkit</strong><small>28h + budget</small></a>
    </div>
  </div>
</section>`;
...
async function enhanceHomeHtml(response, requestUrl) {
  ...
  try {
    ...
    if (!html.includes('href="ebook-library.html"') && html.includes(EBOOK_TOOL_MARKER)) html = html.replace(EBOOK_TOOL_MARKER, EBOOK_TOOL_CARD + EBOOK_TOOL_MARKER);
...
self.addEventListener("install",event=>{self.skipWaiting();event.waitUntil(caches.open(STATIC_CACHE).then(cache=>cache.addAll(STATIC_ASSETS)))});
...
self.addEventListener("fetch",event=>{
  ...
  if(request.mode==="navigate"||(request.headers.get("accept")||"").includes("text/html")){
    event.respondWith(fetch(request).then(async networkResponse=>{const finalResponse=await enhanceHomeHtml(networkResponse,request.url);if(isSameOrigin&&finalResponse&&finalResponse.ok){const copy=finalResponse.clone();caches.open(DYNAMIC_CACHE).then(cache=>{cache.put(request,copy);trimCache(DYNAMIC_CACHE,MAX_DYNAMIC_CACHE_ITEMS)})}return finalResponse}).catch(()=>caches.match(request).then(async cached=>{const fallback=cached||await caches.match("/index.html")||await caches.match("/");return fallback?enhanceHomeHtml(fallback,request.url):fallback})));
    return;
  }
  ...
  event.respondWith(fetch(request).then(r=>{if(isSameOrigin&&r&&r.ok){const c=r.clone();caches.open(DYNAMIC_CACHE).then(cache=>{cache.put(request,c);trimCache(DYNAMIC_CACHE,MAX_DYNAMIC_CACHE_ITEMS)})}return r}).catch(()=>caches.match(request)));
});